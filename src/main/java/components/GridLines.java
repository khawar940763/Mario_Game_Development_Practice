...
public class GridLines extends Component {
    ...
    Vector3f colorForGrid = new Vector3f(0.6f);
...
//        int width = (int) projectionSize.x + Settings.GRID_WIDTH * 2;
        ...
        for(int i = 0; i < maxLines ; i++){
            ...
            if( i < numVtLines){
                DebugDraw.addLine2D(new Vector2f(x, firstY) , new Vector2f(x , firstY + height) , colorForGrid);
            }

            if( i < numHzLines){
                DebugDraw.addLine2D(new Vector2f(firstX, y) , new Vector2f(firstX + width , y ) , colorForGrid);
            }
        }
    }
}
